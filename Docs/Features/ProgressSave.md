...
M6의 첫 단계로 획득 능력과 마지막 체크포인트를 버전이 있는 JSON 데이터로
왕복한다. 저장 데이터에는 Unity 오브젝트 참조나 Scene 경로를 넣지 않고
영구 ID와 재시작 좌표만 기록한다.
...
- `GameProgressSaveController`: `persistentDataPath` 파일 입출력과 능력 카탈로그
...
- 방문 구역, 열린 지름길, 수집 보상과 보스 처치 ID를 같은 DTO에 추가한다.
