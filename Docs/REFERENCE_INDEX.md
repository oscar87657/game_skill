> 보존 자료: 이전 통합 프로토타입의 구현·문서 인덱스입니다. 아래 완료 표시는 당시 기준이며, 새 비교 실험의 완료를 뜻하지 않습니다. 현재 학습 순서는 [실험 인덱스](FEATURE_INDEX.md)를 따릅니다.
...
- `분석 완료`: 코드·대안·테스트 분석은 끝났고 시각 증거를 기다리는 상태
...
|---:|---|---|---|---|
| 01 | [이동](Features/01-Movement.md) | 입력과 물리 기반 | 탐험 가능한 거리와 조작 문법 | 연구 완료 |
| 02 | [전투](Features/02-Combat.md) | 이동 상태와 방향 | 이동을 유지하는 공격 리듬 | 분석 완료 |
| 03 | [체크포인트](Features/03-Checkpoint.md) | 체력과 위치 | 탐험 실패 비용의 기준점 | 분석 완료 |
