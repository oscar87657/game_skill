...
`SideScrollerMotor`가 Unity 입력·상태 타이머·CharacterController 이동을
실행하고, `MovementMath`가 입력 방향과 점프 속도를 계산합니다. 영상은 가속
이동과 지상 대시 뒤 `점프 → 2단 점프 → 공중 대시 → 착지`가 같은 이동
계약에서 자연스럽게 연결되는 결과를 보여줍니다.
...
애니메이션으로 재생됩니다. 대시 속도는 최고 속도의 72%에서 시작해 중앙에서
100%에 도달하고 다시 72%로 끝나는 사인 완화 곡선으로 적용됩니다. 점프는
기본 점프 후 한 번 더 사용할 수 있으며,
